...
tags:
  ...
  - R
  - crime
coverImage: /img/main/thomas-young-634092-unsplash_alt.jpg
coverCaption: _Photo by [Thomas Young](https://unsplash.com/@tryoung) on [Unsplash](https://unsplash.com/)_
coverSize: partial
...
Since there aren't any missing values, I'll move on to checking for duplicates and typos (or _duplicates caused by typos_) in the state and crime columns. Below, we identify 35 unique states (38 less 3 totals) and 12 unique crimes (also excluding total crime).
...
total_by_st_plot <- df %>%
  filter(crime_head == "Total Crimes Against Children", 
         state_ut %in% high_crime_states$state_ut) %>%
  ggplot(aes(x=year, y=df, fill=state_ut, text = paste0("Year: ", year,"\nTotal: ", df))) +
  geom_bar(stat='identity') + 
  facet_wrap(~state_ut) + 
