...
df %>%
  ...
  labs(y = NULL, x = NULL,
       title = "Number of Crimes in Tamil Nadu",
       subtitle = "Based on 5 Most Significant Types of Crime in 2012") +
...
df %>%
  ...
  geom_text(aes(y = df, x = state_ut, label = df), 
            nudge_y = 200, hjust = 0, family = my_font, color = "#22211d", size = rel(0.9)) +
...
level_data %>% 
  ...
  ggplot(aes(x=Crime,y=State, z=bin)) +
    ...
    theme(axis.text.x = element_text(angle=90, hjust=1),
          ...
          legend.position = "top") +
  scale_fill_manual(values = ghibli_palettes$MononokeMedium, name = "No. of Crimes (log scale)",
...
growth.tbl %>%
  ...
  coord_flip() +
    geom_text(aes(y = growth, x = seq(15,1), label = paste0(round(growth),"%")), 
              nudge_y = -1, color = my_bkgd, family = my_font) +
...
```{r map_2001_2012_alt, message=FALSE, warning=FALSE}
...
final.plt.01_12 %>%
  ...
  ggplot() +
  geom_polygon(aes(x = long, y = lat, fill = count, group = group), color = "white", size = 0.25) + 
  coord_map() +
  scale_fill_gradient(name="No. of\nCrimes", limits=c(0,12000), low="#ede5cf", high="#54203F") +
...
cnames$state <- str_replace(cnames$state, "\\s$","\\n")
