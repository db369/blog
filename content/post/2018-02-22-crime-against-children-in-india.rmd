...
It should be noted that there generally isn't any information about how this data was collected. There are certain crimes that appear more prevalent across all states and some for which there is no account. Perhaps people are less likely to report some crimes and more likely to report others.  
...
df %>%
  ...
  arrange(desc(CAGR))
```
Note that 'Procuration Of Minor Girls' is __Inf__ since it was 0 in 2001. Kidnappings have grown by almost 50% a year! 

To add a little more context, I'll take a look at kidnapping and abductions by state. 
```{r kidnapping by state all, fig.height=6, fig.width=9}
...
```{r kidnapping by state 12, message=FALSE, warning=FALSE}
...
The next question I have is _what crimes are most significant in each state?_ A heatmap might be the best way to visualize this.
```{r heatmap, fig.height=9}
df %>%
  gather("year", df, -state_ut, -crime_head) %>%
  filter(year == '2012', crime_head != "Total Crimes Against Children") %>% 
  ggplot(aes(x=crime_head,y=reorder(state_ut, desc(state_ut)), z=df)) +
    geom_tile(aes(fill = df)) + 
...
Again, I'm interested in average annual growth, but here I take a look at total crimes by state. Tamil Nadu comes out on top. That is likely because we're dealing with smaller numbers, but the trajectory is still quite steep. Uttar Pradesh had an average annual growth in crime of about 6% from 2001 to 2012, but crime fell from 2001 to 2002. Average growth from 2002 to 2012 was about `r df %>% gather("year", df, -state_ut, -crime_head, convert = F) %>% filter(crime_head == "Total Crimes Against Children", year %in% c("2002", "2012"), state_ut == "Uttar Pradesh") %>% summarize(growth = paste0(round(100 * ((df[year == 2012] / df[year == 2002]) ^ (1/11) - 1),1),"%"))`, which is twice as fast as indicated, but still places low on the chart below.
...
```{r Map Prepartion, message=FALSE, warning=FALSE}
